...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val minAccount : AcquiredAccount? = acquire(min, this)
            val maxAccount : AcquiredAccount? = acquire(max, this)
            if (minAccount != null && maxAccount != null) {
                val fromAccount = if (min == fromIndex) minAccount else maxAccount
                val toAccount = if (min == toIndex) minAccount else maxAccount

                when {
                    amount > fromAccount.amount -> errorMessage = "Underflow"
                    toAccount.amount > MAX_AMOUNT - amount -> errorMessage = "Overflow"
                    else -> {
